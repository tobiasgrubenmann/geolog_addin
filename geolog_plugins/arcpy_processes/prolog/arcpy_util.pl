:- module(arcpy_util, [extract_selection/2, get_single_features/2, get_single_feature/3, new_in_memory_fc_name/1,
    layer_name/2, add_layer/2, feature_layer/1, feature_class/1, single_feature/1, feature_to_geometry/2, geometry_to_feature/2,
    point_feature/1]).
...
extract_selection(Layer, Selection) :-
    ...
    arcpy_core:arcpy_CopyFeatures_management([Layer, Selection]),
    arcpy_core:arcpy_SelectLayerByAttribute_management([Layer, "CLEAR_SELECTION"]).
...
get_single_features(FeatureClassOrLayer, Feature) :-
    arcpy_core:arcpy_Describe([FeatureClassOrLayer], Description),
    geolog:get_attribute(Description, "OIDFieldName", FieldName),
    arcpy_core:arcpy_da_SearchCursor([FeatureClassOrLayer, [FieldName]], Cursor),
    setup_call_cleanup(geolog:call_method(Cursor, "__enter__"),
                       (
                           ...
                           arcpy_core:arcpy_Select_analysis([FeatureClassOrLayer, Feature, FilterExpression])
                       ),
                       (
...
layer_name(Name, LayerName) :-
    nonvar(Name),
    arcpy_core:arcpy_Exists([LayerName], Exist),
    Exist = false,
    LayerName = Name.
...
add_layer(Features, LayerName) :-
    ...
    arcpy_core:arcpy_MakeFeatureLayer_management([Features, UniqueLayerName]),
    arcpy_core:arcpy_mapping_Layer([UniqueLayerName], Layer),
    arcpy_core:arcpy_mapping_MapDocument(["CURRENT"], MXD),
    geolog:get_attribute(MXD, "activeDataFrame", DF),
    arcpy_core:arcpy_mapping_AddLayer([DF, Layer]).
...
feature_layer(LayerName) :-
    ...
    arcpy_core:arcpy_Exists([LayerName], Exist),
    Exist = true,
    arcpy_core:arcpy_Describe([LayerName], Description),
...
feature_layer(LayerName) :-
    var(LayerName),
    arcpy_core:arcpy_mapping_MapDocument(["CURRENT"], MXD),
    arcpy_core:arcpy_mapping_ListLayers([MXD], Layers),
...
feature_class(FeatureClass) :-
    nonvar(FeatureClass),
    arcpy_core:arcpy_Exists([FeatureClass], Exist),
    Exist = true,
    arcpy_core:arcpy_Describe([FeatureClass], Description),
    geolog:get_attribute(Description, "dataType", Type),
    Type = "FeatureClass".
...
single_feature(Feature) :-
    arcpy_core:arcpy_GetCount_management([Feature], Counts),
...
feature_to_geometry(Feature, Geometry) :-
    arcpy_core:arcpy_da_SearchCursor([Feature, ["SHAPE@"]], Cursor),
    setup_call_cleanup(geolog:call_method(Cursor, "__enter__"),
...
geometry_to_feature(Geometry, Feature) :-
    ...
    arcpy_core:arcpy_CreateFeatureclass_management(["in_memory", Name], Feature),
    arcpy_core:arcpy_da_InsertCursor([Feature, "SHAPE@"], Cursor),
    setup_call_cleanup(geolog:call_method(Cursor, "__enter__"),
...
point_feature(FeatureClassOrLayer) :-
    ...
    arcpy_core:arcpy_Exists([FeatureClassOrLayer], Exist),
    Exist = true,
    arcpy_core:arcpy_Describe([FeatureClassOrLayer], Description),
...
filter_fc_and_copy(Input, FieldName, Value, FilteredOutput) :-
    ...
    arcpy_core:arcpy_Select_analysis([Input, FilteredOutput, FilterExpression]).
%------------------------------------------------------------------------------
